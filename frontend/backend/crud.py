...
def bulk_create_appointments_lenient(
    ...
    # Pre-fetch all appointments for affected dates to avoid N+1 queries
    affected_dates = {e["date"] for e in entries}
    day_appts_map = {
        d: get_appointments_by_date(db, d)
        for d in affected_dates
    }
    ...
    for e in entries:
        ...
        # conflict check in-memory using pre-fetched appointments
        conflicts = [
            a for a in day_appts_map.get(d, [])
            if _overlaps(s, t, a.start_time, a.end_time)
        ]
        if conflicts:
            skipped.append({
                "date": d,
        ...
        db.add(appt)
        created.append(appt)
        # Update in-memory map so subsequent entries on same day see this one
        day_appts_map.setdefault(d, []).append(appt)
...
def update_appointment(
    ...
    **kw,
    ...
    # Only touch `.query` if the column exists on this model and a new value was provided
    if hasattr(appt, "query") and query is not None:
        appt.query = query
...
def list_reminders(
    ...
    *,
    ...
    q = db.query(Reminder).options(joinedload(Reminder.appointment))
